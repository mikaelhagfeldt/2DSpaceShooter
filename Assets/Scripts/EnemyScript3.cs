...
public class EnemyScript3 : MonoBehaviour
{
    ...
    private float field_float_laserCounter;
    ...
    [SerializeField] private AudioClip field_audioClip_enemyLaserSound;
    [SerializeField] private float field_float_enemyLaserSoundVolume = 0.2f;

    [SerializeField] private int field_int_scorePoints = 100;
    ...
    private void triggerEnemyLaserSound()
    {
        AudioSource.PlayClipAtPoint(field_audioClip_enemyLaserSound, Camera.main.transform.position, field_float_enemyLaserSoundVolume);
    }

    private void addScorePoints()
    {
        FindObjectOfType<GameSessionScript>().AddScorePoints(field_int_scorePoints);
    }
}
